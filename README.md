...
この章では、「React Router」を使って**URL と画面コンポーネントを対応づける方法**を学びます。

解説はすべて**「ファイル（画面）単位」**で行い、アプリの画面遷移を実装していきます。

まずは、「URLが変わると、表示される画面も切り替わる」という感覚をつかむことが、この章の目的です。
...
React Routerは、「URLに応じて、どのReactコンポーネントを表示するかを決める」ためのライブラリです。
...
Supabase は、認証・データベース・ストレージなどをまとめて提供する
**BaaS（Backend as a Service）**です。

本章では、Supabase を利用して
ユーザー登録機能を実装するための準備を行います。

まだ Supabase のアカウントを所持していない場合は、
以下の公式サイトからアカウントを作成してください。

[https://supabase.com/](https://supabase.com/)

### 8.2 Supabase プロジェクトを作成
...
### 8.4 SupaBase の Auth Provider（Email認証）の設定

次に、ユーザー登録で使用する Email認証 を有効化します。

1. dashboard 左メニューから「Authentication」を選択します。
...
Reactアプリから Supabase に接続するための
共有クライアント定義ファイルを作成します。

このファイルはSupabaseへの接続設定を1か所にまとめ、アプリ全体で利用するクライアントを定義します。

以下のコマンドを実行し、共通クライアント定義ファイル（src/lib/supabase.ts）を作成します。
...
`createClient`関数には以下の2つの引数を指定することでSupabaseクライアントを初期化します。
...
URLやAPIキーを直接コードに書かず、環境変数として管理します。
...
Viteを使用しているため、環境変数名は、`VITE_`プレフィックス必須です。
...
2. Connect to your project セクションにある「Project API」を確認します。
...
注意 `service_role key` はサーバー専用のため、絶対に使用しません。
...
* `.env` を変更した場合は開発サーバーを再起動します。
* anon public key は公開されても問題ありません。
* `service_role key`は絶対にフロントエンドで使わないでください。

ここまででSupabaseを使った認証機能は完了です。

次の章では、ユーザー登録（サインアップ）処理を実装していきます。

## 9. Supabase Auth を使ったユーザー登録（サインアップ）機能の実装

### 9.1 認証ロジックの設計（Repository パターン）

アプリのロジックは大きく次の2つに別れます。

* 認証まわりの機能
* ノートまわりの機能

これらのロジックは、機能単位で管理するため、`modules`フォルダに格納します。
...
`form`の`onSubmit`を`handleSignup`に置き換え
...
ブラウザーで[http://localhost:5173/signup](http://localhost:5173/signup)を開きテスト用の入力値を入れる。
